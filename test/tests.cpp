...
namespace {
    std::int64_t get_section_load_bias(std::filesystem::path path,
        ...
        while (getline(&line, &len, pipe) != -1) {
            ...
            if (std::regex_search(line, groups, text_regex)) {
                ...
                // Check address is inside the section
                if (address <= file_addr and file_addr < (address + size)) {
                    free(line);
                    pclose(pipe);
        ...
        pclose(pipe);
        sdb::error::send("Could not find section laod bias");
    }
    ...
    virt_addr get_load_address(pid_t pid, std::int64_t offset) {
        ...
        while (std::getline(maps, data)) {
            ...
            if (groups[2] == 'x') { // it is an executable section
        ...
        sdb::error::send("Could not find laod address");
    }
