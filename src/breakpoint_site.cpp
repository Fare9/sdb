...
namespace {
    /// the ids can be non-repeated numbers, so we
    /// can make them sequential, we create a function
    /// in an unnamed namespace so we create different
    /// functions with same name in other parts of the
    /// code.
    auto get_next_id() {
        // the value will be updated in every call
...
void sdb::breakpoint_site::enable() {
    ...
    // instruction used for setting a software breakpoint in x86
