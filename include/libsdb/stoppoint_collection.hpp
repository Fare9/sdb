...
namespace sdb {
    /// As we will have different type of breakpoints
    /// let's create a template that provides the same
    /// functionalities to get them, enable/disable them
    /// or removing them.
    ...
    public:
        ...
        /// We try to access the id_type from the Stoppoint class
        /// if the class does not have this type, we will have a
        /// compilation error
        ...
        /// Helpers to implement a for_each, the user
        /// can provide a function that will be used
        /// to be run on each breakpoint
        template <class F>
        void for_each(F f);
        template <class F>
    ...
    // FIX 1: missing '>' before '::' in template specialization
    template <class Stoppoint>
    ...
    // FIX 2: missing '>' before '::' in template specialization
    template <class Stoppoint>
    ...
    // FIX 5: was calling find_by_address(id) instead of find_by_id(id)
    template <class Stoppoint>
    ...
    // FIX 6: was calling find_by_address(id) instead of find_by_id(id)
    template <class Stoppoint>
